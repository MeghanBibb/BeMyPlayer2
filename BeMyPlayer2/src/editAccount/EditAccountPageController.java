...
public class EditAccountPageController extends PageController {
	...
	public void actionPerformed(ActionEvent e) {
		...
		case BACK: // go back to profile page
			logger.info("Back was preformed");
		...
		case CANCEL:// go back to edit page
			logger.info("Cancel was preformed");
		...
		case SUBMITEDITACCOUNT:
			logger.info("SubmitAccount was preformed");
			if (validateCreatePage1() == true) {
				ClientManager.getActiveAccount().getAccountProfile()
						.setUsername(this.getEditAccountModel().getFrmtdtxtfldEnterUsername().getText());
				ClientManager.getActiveAccount()
						.setSecurityQ1(this.getEditAccountModel().getSecurityQ().getSelectedItem().toString());
				ClientManager.getActiveAccount().getAccountProfile()
						.setGender(this.getEditAccountModel().getGenderBox().getSelectedItem().toString());
				if (!this.getEditAccountModel().getPwdEnterPass().getText().isEmpty()) {
					ClientManager.getActiveAccount()
							.setPasswordHash(Hasher.hashString(this.getEditAccountModel().getPwdEnterPass().getText()));
				}
				try {
				...
				if (!this.getEditAccountModel().getSecQA().getText().isEmpty()) {
					ClientManager.getActiveAccount()
							.setSecurityQ1AnsHash(Hasher.hashString(this.getEditAccountModel().getSecQA().getText()));
				}
		...
		case SUBMITEDITQUESTIONAIRE:
			logger.info("SubmittedQuestionaire was preformed");
			if (validateCreatePage2() == true) {
				ClientManager.getActiveAccount().getAccountProfile()
						.setPlatforms(this.getEditAccountModel().getPlatforms());
		...
		case SUBMITEDITPROFILE:
			logger.info("SubmittedProfile was preformed");
			if (validateCreatePage3() == true) {
				...
				desc += "<HTML>";
				ClientManager.getActiveAccount().getAccountProfile()
						.setProfilePicture(this.editAccountModel.getProfileImg());
				...
				try {
					...
					if (ClientManager.getActiveAccount().getAccountProfile().getProfilePicture() == null) {
						ClientManager.updateProfileImage(ResourceManager.loadImage("defaultIcon.png"),
								ClientManager.getActiveUserID());
					} else {
						ClientManager.updateProfileImage(
								ClientManager.getActiveAccount().getAccountProfile().getProfilePicture(),
								ClientManager.getActiveUserID());
					}
				} catch (Exception e1) {
		...
		case PROFILE:// go to edit profile page
			logger.info("Go to Edit Profile Page was preformed");
			this.editAccountPanel.removeAll();
		...
		case ACCOUNT: // go to edit account page
			logger.info("Go to Edit Account Page was preformed");
			this.editAccountPanel.removeAll();
			GraphicsController.getMainFrame().repaint();
		...
		case QUESTIONNAIRE: // go to questionnaire page
			logger.info("Go to Questionnaire Page was preformed");
			this.editAccountPanel.removeAll();
			GraphicsController.getMainFrame().repaint();
			EditAccountPageView.launchEditQuestionnairePage(this, copyFrame);
			break;
		case UPGRADE: // go to upgrade account page
			logger.info("Go to Upgrade Page was preformed");
			try {
				payment.PaymentInfo p = ClientManager.getPaymentInfo(ClientManager.getActiveUserID());
				if (p != null) {
					logger.info("GOT PAYMENT INFO FROM DB\n");
				}
			} catch (DBFailureException e1) {
				logger.severe("Database failed to pull payment info for " + ClientManager.getActiveUserID());
				InvalidPopup p = new InvalidPopup(this.getEditAccountPanel(),
						"Error loading payment info from database");
			}

			GraphicsController.processPage(PageCreator.PAYMENT_PAGE, backPage);
			break;
		case END_PAYMENT:
			logger.info("End Payment was preformed");
			/* make popup warning */
			int dialogButtonP = JOptionPane.YES_NO_OPTION;
			int dialogResultP = JOptionPane.showConfirmDialog(this.editAccountPanel,
					"Are you sure you want to cancel your payment plan?\n You will no longer benefit from being a pro account.",
					"Unsubscribe from pro features?", dialogButtonP);
			if (dialogResultP == 0) {
			...
			break;
		case MUTE:
			logger.info("Mute was preformed");
			...
			int dialogResult = JOptionPane.showConfirmDialog(this.editAccountPanel, msg, "Account Muting",
					dialogButton);
			...
			if (dialogResult == 0) {
				...
				if (muteStatus) {
					try {
						...
						ClientManager.updateProfile(ClientManager.getActiveAccount().getAccountProfile());
						logger.info("Un-muted account "
								+ ClientManager.getActiveAccount().getAccountProfile().getUsername());
					...
					try {
						...
						ClientManager.updateProfile(ClientManager.getActiveAccount().getAccountProfile());
						logger.info(
								"Muted account " + ClientManager.getActiveAccount().getAccountProfile().getUsername());
			...
			break;
		case DELETE:
			logger.info("Delete Account was preformed");
			JPasswordField pf = new JPasswordField();
			int opt = JOptionPane.showConfirmDialog(null, pf, "Type your password to delete account",
					JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
			// String m = JOptionPane.showInputDialog("Type your password to delete
			// account");
			// dialogButton.int dialogResult2=
			// JOptionPane.showConfirmDialog(this.editAccountPanel, "Are you sure you want
			// to delete your account?","Delete account?", dialogButton2);
			String m = pf.getText();
	...
	public boolean validateCreatePage1() {
		boolean valid = true;
		logger.info("Validating Create Page 1");

		// gamer tag, password, revalidate password, validate security question,
		// validate answer, validate gender and dob
		// validation needed
		/*
		 * no sql commands no empty limit size
		 * 
		 * age > 0 && less than 100
		 * 
		 *//*
		...
		try {
			...
			if (this.editAccountModel.getPwdEnterPass().getText().equalsIgnoreCase("")) {
				...
				} else if (!this.editAccountModel.getPwdValidatePass().getText()
						.equals(this.editAccountModel.getPwdEnterPass().getText())) {
					valid = false;
					warnings.add("Passwords do not match\n");
				}
			}
			if (this.editAccountModel.getSecQA().getText().equalsIgnoreCase("") && !this.editAccountModel.getSecurityQ()
					.getSelectedItem().equals(ClientManager.getActiveAccount().getSecurityQ1())) {
				valid = false;
	...
	public boolean validateCreatePage2() {
		boolean valid = true;
		logger.info("Validating Create Page 2");
		int countPlat = 0;
		int countGenre = 0;
	...
	public boolean validateCreatePage3() {
		logger.info("Validating Create Page 3");
